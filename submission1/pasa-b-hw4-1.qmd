...
if 'ma_rate' not in df.columns:
    plt.text(0.5, 0.5, 'No benchmark payment data found',
             ha='center', va='center')
else:
    # Convert to numeric and clean
    df['ma_rate'] = pd.to_numeric(df['ma_rate'], errors='coerce')
    benchmark_by_year = df.groupby('year', as_index=False)['ma_rate'].mean()

    # Ensure all years from 2010 to 2015 are present
    all_years = pd.DataFrame({'year': range(2010, 2016)})
    benchmark_by_year = pd.merge(all_years, benchmark_by_year, on='year', how='left')
    ...
    # Calculate and annotate change from 2010 to 2015
    if not pd.isna(benchmark_by_year.loc[benchmark_by_year['year'] == 2010, 'ma_rate'].values[0]) and \
       not pd.isna(benchmark_by_year.loc[benchmark_by_year['year'] == 2015, 'ma_rate'].values[0]):
        
        start_val = benchmark_by_year.loc[benchmark_by_year['year'] == 2010, 'ma_rate'].values[0]
        end_val = benchmark_by_year.loc[benchmark_by_year['year'] == 2015, 'ma_rate'].values[0]
        percent_change = ((end_val - start_val) / start_val) * 100

        note = f'Change from 2010 to 2015: ${end_val - start_val:.2f} ({percent_change:.1f}%)'
        plt.text(2012, benchmark_by_year['ma_rate'].max() * 0.9 if benchmark_by_year['ma_rate'].max() else 800, 
                 note, bbox=dict(facecolor='white', alpha=0.8))

    # Print the data for verification
    print("\nAverage Benchmark Payment by Year (including missing):")
    print(benchmark_by_year.to_string(index=False, formatters={'ma_rate': '${:,.2f}'.format if pd.notna else str}))

plt.savefig('benchmark_payment_trend_fixed.png', dpi=300)
plt.show()
